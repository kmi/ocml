...
;(defun reload-this-ontology-and-its-dependents (onto)
;  "this function does not use the load file of an ontology.  It assumes that 
;   this only contains the definition of the ontology"
;  (setf (ontology-directory onto) 
;        (make-ontology-directory))
;  (finalize-ontology (name onto) 
;                    onto
;                     (ontology-includes onto)
;                     (ontology-files onto)
;                     (ontology-pathname onto)
;                     nil nil)
;  (loop for dep-onto in (ontology-included-by onto)
;        do
;        (reload-this-ontology-and-its-dependents dep-onto)))
   

;(defun reconstruct-ontology (ontology subs)
;  (clear-all)
;  (loop for from-onto in subs
;          do
;          (copy-ontology from-onto ontology)))
