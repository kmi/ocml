...
(defun compile-bc-rule (name rule &optional (trace *compile-trace*))
  "Compile a bc rule generating a lisp function. 
   If the correspending relation has a sufficient
...
(defmacro call-bc-rule (goals &optional all env query-mode? compile (in-line t))
  (let* ((*compile* compile)
	 (*in-line* in-line)
  ...
  (let ((*in-line* noinline))
    ...
    (unless nocompile
	    (loop for name in *compiled-rules*
		  do (unless (compiled-function-p name)     ;; this is needed because
			   (compile name))))))              ;; there functions 
                                                            ;; which are compiled
                                                            ;; at run time
(defun clear-all-bc-rules ()
  (loop for name in *compiled-rules*
	do (fmakunbound name)
  (setq *compiled-rules* nil)))
