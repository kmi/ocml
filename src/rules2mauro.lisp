...
(defun get-rule-packet (name)
  (right-value name *rule-packets*))

(defun add-rule-packet (name packet)
  (if *rule-packets*
      (nconc *rule-packets*
             (list (cons name packet)))
      (setf *rule-packets* (list (cons name packet)))))

(defun all-rule-packets ()
  (mapcar #'cdr *rule-packets*))
...
(defun get-or-create-packet (name)
  (or (get-rule-packet name)
      (new-rule-packet name)))

(defun new-rule-packet (name)
  (let ((p (make-rule-packet :name name)))
    (add-rule-packet name p)
    p))

(defun make-default-rule-packet ()
  (make-rule-packet :name :default-rule-packet))
...
(defun remove-fc-rules-in-all-packets ()
  (loop for pair in *rule-packets*
        do
        (remove-all-fc-rules-in-packet (cdr pair))))
...
(defun find-fc-rule (name &optional packet)
  (if packet
      ...
      (loop for pair in *rule-packets*
        for result = (find-fc-rule-in-packet name (cdr pair))
        until result
...
(defun add-backward-rule (name documentation clauses &optional arity)
  (Let* ((rule (make-instance 'backward-rule
                             :name name
                             :clauses (mapcar #'make-bc-rule-clause clauses)
			     :documentation documentation))
         (schema (caar clauses))
         (rel (or (car schema) name)))
    (add-bc-rule name rule)
    (setf (defines-relation rule) rel)
    (set-defined-by-rule-entry
     (find-or-create-relation rel (or arity (length (cdr schema))))
                               rule)
    rule))

(defun make-bc-rule-clause (clause)
  (make-instance 'bc-clause
...
(defmethod remove-rule ((rule backward-rule))
  (with-slots (name defines-relation) rule
    (remove-bc-rule name)
    (reset-defined-by-rule-entry (get-relation defines-relation))))
