...
(defclass ocml-function (name-mixin lisp-attachment-mixin ;;;onto-spec-mixin
  ...
  ((trace-depth :initform 0 :accessor fun-trace-depth)
   ...
   (definition :initarg :def :initform nil)
...
(defmethod initialize-instance :after ((fun ocml-function) &rest initargs)
  ...
  (with-slots (name schema arity user-defined? lisp-fun) fun
    (when (get-function name)
      (warn "Redefining function ~S"name))
    (record-source-file name (type-of fun)) ;;;;;'ocml-function)
...
(defun ocml-apply-internal (fun args &optional env (eval-fun #'ocml-eval-fun ))
 ...
 ;;;;               "The first argument to APPLY cannot be a procedure...when evaluating ~S in ~S"
 ;;;                    (cons fun args)
 ;;;;                    (relevant-env (cons fun args) env)))))))

       
;;;;(defun ocml-apply-internal (fun args &optional env)
;;;;  (ocml-eval-fun-term fun (mapcar #'(lambda (arg)
;;;;                                              (list 'quote arg))
;;;;                                          args) env))
...
;;;EVALUATE-TELL-FORM ---
;(defun evaluate-tell-form (exp env )
;  (tell1 exp nil env))
     ;;;;(tell1 (parse-assertion-in-function-body exp env) nil env))

;;;EVALUATE-UNASSERT-FORM
;(defun evaluate-unassert-form (exp env)
;    (unassert1 (parse-assertion-in-function-body exp env)env))
