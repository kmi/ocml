...
(defmethod add-slot-value-to-instance ((instance basic-domain-class)
                                       slot value &optional stop-renamed-chain?
                                       )
  (cond (stop-renamed-chain?
         (Let ((renaming-slot (find-renaming-slot-in-instance  slot instance)))
           (when renaming-slot
             (add-slot-value-to-instance-int instance renaming-slot value)
             (add-slot-value-to-instance instance renaming-slot value t))))
        (t
          (let ((renamed-slot (find-renamed-slot-in-instance  slot instance)))
             (cond (renamed-slot
                    (add-slot-value-to-instance instance renamed-slot value))
                   (t
                    (add-slot-value-to-instance-int instance slot value)
                    (add-slot-value-to-instance instance slot value t)))))))

(defmethod add-slot-value-to-instance-int ((instance basic-domain-class)
                                           slot value 
                                           &aux new-values change? discarded-values)
...
;;;Changed to cope with renamed slots - Enrico 17/2/99
(defmethod maybe-remove-slot-value ((instance basic-domain-class)slot value &optional no-checks)
   (destructuring-bind (all-values &optional local-values flag)
       (or (slot-value instance slot)
           '(nil))
     flag ;;;ignore
     (when (member value local-values :test #'equal)
       ;;;if this slot is member of a renaming chain, we remove all of them
       (Let ((chain (find-renaming-chain  slot
                                         (renaming-chains (parent-class instance)))))
         (if chain
           (loop for each-slot in chain
                 do
                 (remove-slot-value instance each-slot value all-values local-values ;;;;flag 
                          no-checks))
           ;;no renaming chain, simple one-slot removal
           (remove-slot-value instance slot value all-values local-values ;;;;flag 
                          no-checks))))))
...
(defmethod remove-local-slot-values ((instance basic-domain-class)slot
  ...
  (Let ((chain (find-renaming-chain  slot 
    ...
    (if chain
      (loop for each-slot in chain
            do
            (remove-local-slot-values-int instance each-slot no-checks))
      (remove-local-slot-values-int instance slot no-checks))))
